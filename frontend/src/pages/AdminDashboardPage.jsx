...
const fallbackRecentBookings = [
  { user: "Ava Chen", car: "Toyota RAV4", dates: "May 20 - May 22", status: "CONFIRMED" },
  { user: "Liam Brown", car: "Honda Civic", dates: "May 21 - May 24", status: "PENDING" },
  { user: "Noah Patel", car: "Nissan Rogue", dates: "May 23 - May 26", status: "IN_PROGRESS" },
  { user: "Emma Wong", car: "Ford F-150", dates: "May 25 - May 27", status: "CONFIRMED" },
  { user: "Sophia Li", car: "Toyota Corolla", dates: "May 26 - May 29", status: "COMPLETED" },
];
...
export default function AdminDashboardPage() {
  ...
  const [locationMode, setLocationMode] = useState("hub");
  ...
  const activePageTitle = useMemo(() => pageTitles[activeTab] || "Admin Dashboard", [activeTab]);
  ...
  useEffect(() => {
    if (!addressQuery.trim() || !autocompleteServiceRef.current) {
      setPlacePredictions([]);
  ...
  const recentBookings = useMemo(() => {
    if (!bookings.length) return fallbackRecentBookings;
    return bookings.slice(0, 5).map((booking) => ({
      user: `User #${booking.renterUserId || "n/a"}`,
  ...
  const createListing = async (event) => {
    ...
    try {
      const usesCompanyDropdown = locationMode === "hub";
      const response = await apiPost(
        "/api/owner/listings",
        {
          ...
          cityZone: usesCompanyDropdown ? undefined : listingForm.cityZone,
        },
        true,
      );
      const createdListingId = response?.listing?.listingId;
      let uploadedCount = 0;
      if (createdListingId && pendingPhotoFiles.length) {
        const uploadResult = await uploadListingPhotos(createdListingId, {
          skipRefresh: true,
          skipSuccess: true,
        });
  ...
  const selectAddressPrediction = async (prediction) => {
    ...
    geocoderRef.current.geocode({ placeId: prediction.place_id }, (results, status) => {
      if (
        ...
        const formatted = results[0].formatted_address || prediction.description;
    ...
    <Layout>
      <div className="min-h-screen bg-gray-50 flex">
        ...
        <div className="p-6 border-b border-gray-200">
          <p className="text-xl font-bold text-gray-900">DriveBnb Admin</p>
        </div>
          ...
          {activeTab === "overview" && (
            <>
              <section className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-6 p-10">
                ...
                />
                <AnalyticsCard label="Fleet Utilization" value="78%" />
              </section>
                ...
                <table className="w-full text-left border-collapse">
                  ...
                  <tbody>
                    {recentBookings.map((row, idx) => (
                      <tr key={`${row.user}-${idx}`} className="border-b border-gray-100 hover:bg-gray-50 transition">
          ...
          {activeTab === "create_listing" && (
            <section className="pb-10">
              <div className="px-10 pt-10">
                <h2 className="text-2xl font-semibold text-gray-900">Create Company Listing</h2>
              </div>
