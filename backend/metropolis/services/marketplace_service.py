...
def _resolve_initial_booking_status(source_type: str, instant_book: bool) -> str:
    if source_type == "FLEET":
        return "CONFIRMED"
    if source_type == "OWNER" and instant_book:
        return "CONFIRMED"
    ...
    payload.update(
        {
            ...
            "listingLocation": {
                ...
                "lng": float(lng) if lng is not None else None,
                "cityZone": row.get("city_zone"),
                "address": row.get("raw_address") or row.get("pickup_address"),
    ...
    def create_booking(self, renter_user_id: int, payload: dict) -> dict:
        with get_connection() as conn:
            with conn.cursor(cursor_factory=RealDictCursor) as cur:
                ...
                initial_status = _resolve_initial_booking_status(
                    listing["source_type"],
                    bool(listing.get("instant_book", True)),
                )
    ...
    def cancel_booking(self, booking_id: int, renter_user_id: int) -> dict:
        with get_connection() as conn:
            with conn.cursor(cursor_factory=RealDictCursor) as cur:
                ...
                if row["status"] not in {"PENDING_APPROVAL", "CONFIRMED"}:
                    return {
                        "status": "validation_error",
