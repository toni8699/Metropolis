...
from metropolis.services import fleet_service, listing_service
...
def get_analytics(query):
    ...
    if scope == "owner":
        return listing_service.owner_analytics(g.current_user["userId"])
    if scope == "fleet":
        if not g.current_user.get("isAdmin"):
            raise Forbidden(description="Admin access required.")
        return fleet_service.admin_analytics()
