...
class PaymentService:
    ...
    def handle_webhook(self, payload: bytes, signature: str | None) -> dict:
        ...
        if event["type"] == "v2.core.account[requirements].updated":
            ...
            account_id = self._account_id_from_connect_webhook(event)
            if not account_id:
                return {"status": "ignored", "message": "Missing Connect account id."}
            return payout_service.handle_account_updated(account_id)

        return {"status": "ignored", "message": f"Unhandled event type {event['type']}."}

    @staticmethod
    def _account_id_from_connect_webhook(event: dict) -> str | None:
        obj = event.get("data", {}).get("object") or {}
        for key in ("id", "account"):
            val = obj.get(key)
            if isinstance(val, str) and val.startswith("acct_"):
                return val
        related = event.get("related_object") or obj.get("related_object") or {}
        val = related.get("id")
        if isinstance(val, str) and val.startswith("acct_"):
            return val
        return None
    ...
    def _complete_payment(
        ...
        *,
        ...
        from vroom.services.booking_notifications import notify_payment_completed
