...
_logger = logging.getLogger("vroom")
...
async def startup(ctx: dict) -> None:
    if not settings.database_url:
        return
    from vroom.core.db import init_db
...
async def shutdown(ctx: dict) -> None:
    from vroom.core.db import dispose_db
...
async def sweep_expired_bookings(ctx: dict) -> dict:
    ...
    from vroom.services import booking_service
...
async def sweep_expired_trip_inspection_photos(ctx: dict) -> dict:
    ...
    from vroom.services import trip_inspection_service
...
class WorkerSettings:
    """arq worker entrypoint: arq vroom.jobs.booking_sweep.WorkerSettings"""
