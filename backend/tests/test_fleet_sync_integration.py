    ...
    with psycopg2.connect(DATABASE_URL) as conn:
        with conn.cursor() as cur:
            ...
            cur.execute(
                ...
                " VALUES (98001, 'Sync Class', 0) ON CONFLICT (classid) DO NOTHING"
            )
            cur.execute(
...
def test_admin_fleet_sync_idempotent():
    ...
    admin_token, admin_id = _register("fleet-admin-sync", admin=True)

    resp1 = _api("POST", "/api/admin/fleet/sync", token=admin_token)
    assert resp1.status_code == 200, resp1.text

    resp2 = _api("POST", "/api/admin/fleet/sync", token=admin_token)
    assert resp2.status_code == 200, resp2.text

    listings_resp = _api("GET", "/api/admin/fleet/listings", token=admin_token)
...
def test_relocation_simulation_returns_result():
    ...
    resp = _api("POST", "/api/admin/relocation/simulate", token=admin_token)
    assert resp.status_code == 200, resp.text
    body = resp.json()
...
def test_non_admin_cannot_run_relocation():
    ...
    resp = _api("POST", "/api/admin/relocation/simulate", token=token)
    assert resp.status_code == 403, resp.text
