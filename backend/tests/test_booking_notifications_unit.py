...
from metropolis.services.booking_notifications import (
...
def test_notify_payment_completed_confirmed():
    with patch("metropolis.services.mail_service.send_booking_confirmed") as send:
        notify_payment_completed(10, "CONFIRMED")
    send.assert_called_once_with(10)


def test_notify_payment_completed_pending_approval():
    with patch("metropolis.services.mail_service.send_booking_pending_approval") as send:
...
def test_notify_trip_completed_calls_email_and_payout():
    with (
        patch("metropolis.services.mail_service.send_trip_completed_review") as review,
        patch(
            "metropolis.services.payout_service.payout_service.transfer_for_booking",
        ) as payout,
    ):
